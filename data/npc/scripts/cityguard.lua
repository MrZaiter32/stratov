...
selfSay("En esta ciudad no pekearas hijo de puta!")
end
...
selfSay("Te escapaste hijo de puta pero pa la proxima vas a caer duro chtm.")
goToOrigPos()
return
end

if((math.abs(playerPos.x - myPos.x) <= 1) and (math.abs(playerPos.y - myPos.y) <= 1)) then
doTargetCombatHealth(getNpcCid(), target, COMBAT_LIFEDRAIN, -80, -200, CONST_ME_FIREAREA)
lastAttack = os.clock()
end
end