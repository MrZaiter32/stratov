...
function autoloot(cid, target, pos)
    ...
    local function doStack(cid, itemid, new)
        ...
        if count ~= 0 then
            local find = getPlayerItemById(cid, true, itemid, count)
            if find and find.uid > 0 then
                doRemoveItem(find.uid)
            else
                newCount = new
            end
        end
    ...
    for _, item in pairs(items) do
        if not isCreature(item.uid) then
            ...
            if name and name:find(target:lower()) then
                corpse = item.uid
                break
            end
        end
    ...
    if corpse ~= -1 and isContainer(corpse) and getItemAttribute(corpse, "corpseowner") == cid then
        local lootList = tostring(getPlayerStorageValue(cid, stor)):gsub("_", ""):explode(",")
        scanContainer(cid, corpse, lootList)
    end
end
